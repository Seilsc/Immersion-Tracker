...
const STREAK_VISIBLE_KEY = "lang-immersion-streak-visible";

function getStreakVisible() {
  return localStorage.getItem(STREAK_VISIBLE_KEY) !== "false";
}

function applyStreakVisible(visible) {
  const card = document.getElementById("streak-card");
  if (card) card.style.display = visible ? "" : "none";
  const toggle = document.getElementById("toggle-streak");
  if (toggle) toggle.checked = visible;
}

document.getElementById("toggle-streak")?.addEventListener("change", e => {
  const visible = e.target.checked;
  localStorage.setItem(STREAK_VISIBLE_KEY, visible);
  applyStreakVisible(visible);
});
...
renderStats = function() {
  originalRenderStats();
  applyStreakVisible(getStreakVisible());
};
...
refreshGoalConfigUI = function() {
  ...
  const toggle = document.getElementById("toggle-streak");
  if (toggle) toggle.checked = getStreakVisible();
};
