...
function renderKPIs() {
  ...
  el.innerHTML = `
    ...
    <div class="kpi-card">
      <div class="kpi-label">Media por día activo</div>
      <div class="kpi-value">${formatHM(avgSec)}</div>
      <div class="kpi-sub">solo días con sesión</div>
    </div>
    <div class="kpi-card">
    ...
    <div class="kpi-card">
      <div class="kpi-label">Contenido</div>
      <div class="kpi-value">${formatHM(ytSec+showSec+movieSec)}</div>
      <div class="kpi-sub">YT · Series · Pelis</div>
    </div>`;
}
...
function renderStreak() {
  ...
  state.sessions.filter(s => !currentLang || s.lang === currentLang).forEach(s => daySet.add(new Date(s.ts).toDateString()));
  state.youtube.filter(v => !currentLang || v.lang === currentLang).forEach(v => { if (v.ts) daySet.add(new Date(v.ts).toDateString()); });
  state.shows.filter(s => !currentLang || s.lang === currentLang).forEach(s => { if (s.ts) daySet.add(new Date(s.ts).toDateString()); });
  state.movies.filter(m => !currentLang || m.lang === currentLang).forEach(m => { if (m.ts) daySet.add(new Date(m.ts).toDateString()); });
...
function renderThisWeek() {
  ...
  let thisSec = 0, prevSec = 0;
  for (let d=new Date(weekStart); d<=now; d.setDate(d.getDate()+1)) thisSec += dayMap[d.toDateString()]||0;
  for (let d=new Date(prevWeekStart); d<weekStart; d.setDate(d.getDate()+1)) prevSec += dayMap[d.toDateString()]||0;

  document.getElementById("this-week-time").textContent = formatHM(thisSec);
  const vsEl = document.getElementById("this-week-vs");
  if (prevSec > 0) {
    const diff = thisSec - prevSec;
    const sign = diff >= 0 ? "+" : "";
    vsEl.textContent = `${sign}${formatHM(Math.abs(diff))} vs semana anterior`;
    vsEl.style.color = diff >= 0 ? "var(--green)" : "var(--accent)";
  } else {
    vsEl.textContent = prevSec === 0 ? "primera semana registrada" : "";
  }
}
...
function renderHeatmap() {
  ...
  for (let w=0; w<WEEKS; w++) {
    ...
    if (m !== lastMonth) { colLabels.push(weekDate.toLocaleDateString('es-ES',{month:'short'})); lastMonth = m; }
    else colLabels.push('');
  }
  ...
  if (monthLabels) {
    colLabels.forEach(label => {
      ...
      span.style.cssText = `width:17px;text-align:left;overflow:hidden;`;
  ...
  for (let i=0; i<DAY_COUNT; i++) {
    ...
    if (isFuture) cell.style.opacity="0.25";
    if (d.toDateString() === today.toDateString()) cell.style.outline="2px solid var(--accent)";
    cell.title = `${d.toLocaleDateString('es-ES',{weekday:'short',day:'numeric',month:'short'})}: ${secs ? formatHM(secs) : "sin actividad"}`;
    hm.appendChild(cell);
  }
}
