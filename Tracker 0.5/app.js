...
async function addShowFromTmdb(result) {
  ...
  try {
    ...
    state.shows.push({ name: details.name, epDuration, episodesWatched: 0, tmdbId: details.id, lang, activity: "Freeflow Listening", manualMinutes: {}, ts: Date.now() });
    saveState();
    renderAll();
...
async function addMovieFromTmdb(result) {
  ...
  try {
    ...
    state.movies.push({ title: details.title, year: details.release_date?.slice(0,4) || "", seconds: runtime*60, tmdbId: details.id, lang, ts: Date.now() });
    saveState();
    renderAll();
