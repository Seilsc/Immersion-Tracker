...
function renderMediaTimeControls(idx, type, currentSec, autoSec) {
  const key = `${type}-${idx}`;
  const autoTab = autoSec > 0
    ? `<button class="time-mode-tab" id="mtab-auto-${key}" onclick="switchMediaTab('${key}','auto')">Automático (${formatHMS(autoSec)})</button>`
    : '';
  return `
    ...
    <div id="mauto-${key}" style="display:none;align-items:center;gap:0.5rem;flex-wrap:wrap;">
      ...
      <span style="font-family:var(--mono);font-size:11px;color:var(--ink-soft);">duración automática</span>
      <button class="secondary" style="padding:0.4rem 0.8rem;font-size:12px;" onclick="saveMediaAuto('${key}','${type}',${idx},${autoSec})">Guardar</button>
