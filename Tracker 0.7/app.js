...
function getTodaySeconds(lang) {
  ...
  const ytSec = state.youtube.filter(filter).reduce((s,v) => s + v.seconds, 0);
  const actSec = state.sessions.filter(filter).reduce((s,a) => s + a.seconds, 0);
  const movieSec = state.movies.filter(filter).reduce((s,m) => s + m.seconds, 0);
...
function selectActivity(id) {
  ...
  // Default to timer mode; if manualTime activity, switch to manual tab
  if (act.manualTime) {
    setTimeMode("manual");
  } else {
    setTimeMode("timer");
