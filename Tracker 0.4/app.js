...
function loadState() {
  ...
  return { languages: ["Japonés"], sessions: [], youtube: [], shows: [], movies: [] };
}
...
function getTotalSeconds() {
  const ytSec = state.youtube.filter(v => !currentLang || v.lang === currentLang).reduce((s,v) => s + v.seconds, 0);
  const showSec = state.shows.filter(s => !currentLang || s.lang === currentLang).reduce((s, sh) => s + sh.epDuration*60*sh.episodesWatched, 0);
  const movieSec = state.movies.filter(m => !currentLang || m.lang === currentLang).reduce((s,m) => s + m.seconds, 0);
...
function renderTotals() {
  const { ytSec, showSec, movieSec, actSec, grand } = getTotalSeconds();
  document.getElementById("total-grand").textContent = formatHM(grand);
  document.getElementById("total-youtube").textContent = formatHM(ytSec);
  document.getElementById("total-shows").textContent = formatHM(showSec);
  document.getElementById("total-movies").textContent = formatHM(movieSec);
  document.getElementById("total-activities").textContent = formatHM(actSec);
}
...
function selectActivity(id) {
  ...
  if (act.manualTime) {
    document.getElementById("timer-box-wrap").style.display = "none";
    document.getElementById("manual-box-wrap").style.display = "block";
  } else {
    document.getElementById("timer-box-wrap").style.display = "block";
    document.getElementById("manual-box-wrap").style.display = "none";
...
document.getElementById("yt-add").addEventListener("click", async () => {
  ...
  try {
    ...
    const enriched = details.map(d => ({ ...d, lang, activity, ts: Date.now() }));
  ...
  filtered.forEach((video, i) => {
    ...
    entry.innerHTML = `
      <div class="entry-main">
        <span class="entry-title">${video.title}</span>
        <span class="entry-sub">${video.lang || ""} · ${video.activity || ""} · ${video.id}</span>
      </div>
      <div class="entry-right">
        <span class="entry-duration">${formatHMS(video.seconds)}</span>
...
async function addShowFromTmdb(result) {
  ...
  try {
    ...
    state.shows.push({ name: details.name, epDuration, episodesWatched: 0, tmdbId: details.id, lang, activity: "Freeflow Listening", manualMinutes: {} });
    saveState();
    renderAll();
...
async function addMovieFromTmdb(result) {
  ...
  try {
    ...
    state.movies.push({ title: details.title, year: details.release_date?.slice(0,4) || "", seconds: runtime*60, tmdbId: details.id, lang });
    saveState();
    renderAll();
...
function renderMovies() {
  ...
  filtered.forEach(movie => {
    ...
    entry.innerHTML = `
      <div class="entry-main">
        <span class="entry-title">${movie.title || "Película sin título"}</span>
        <span class="entry-sub">${movie.lang || ""} · ${movie.year || ""}</span>
      </div>
      <div class="entry-right">
        <span class="entry-duration">${formatHMS(movie.seconds)}</span>
...
function renderChartByLang() {
  ...
  state.youtube.forEach(v => { langSecs[v.lang] = (langSecs[v.lang]||0) + v.seconds; });
  state.shows.forEach(s => { langSecs[s.lang] = (langSecs[s.lang]||0) + getShowSeconds(s); });
  state.movies.forEach(m => { langSecs[m.lang] = (langSecs[m.lang]||0) + m.seconds; });
...
function renderChartByCat() {
  ...
  yt.forEach(v => { catSecs["YouTube"] = (catSecs["YouTube"]||0) + v.seconds; });
  shows.forEach(s => { catSecs["Series"] = (catSecs["Series"]||0) + getShowSeconds(s); });
  movies.forEach(m => { catSecs["Películas"] = (catSecs["Películas"]||0) + m.seconds; });
...
function renderChartByActivity() {
  ...
  yt.forEach(v => { const name = v.activity || "YouTube"; actSecs[name] = (actSecs[name]||0) + v.seconds; });
...
function renderHeatmap() {
  ...
  state.youtube.filter(v => !currentLang || v.lang === currentLang).forEach(v => { if (v.ts) addToDay(v.ts, v.seconds); });
  ...
  list.innerHTML = allEntries.map(e => `
    <div class="history-entry">
      <div class="history-entry-main">
        <div class="history-entry-title">${e.title}</div>
...
document.getElementById("export-csv").addEventListener("click", () => {
  const rows = [["Título","Categoría","Idioma","Duración (seg)","Fecha"]];
  state.sessions.forEach(s => rows.push([s.activityName, s.cat, s.lang, s.seconds, s.ts ? new Date(s.ts).toISOString() : ""]));
  state.youtube.forEach(v => rows.push([v.title, "YouTube", v.lang||"", v.seconds, v.ts ? new Date(v.ts).toISOString() : ""]));
  state.shows.forEach(s => { const sec = getShowSeconds(s); if (sec) rows.push([s.name, "Series", s.lang||"", sec, ""]); });
  state.movies.forEach(m => rows.push([m.title, "Películas", m.lang||"", m.seconds, ""]));
  const csv = rows.map(r => r.map(c => `"${String(c).replace(/"/g,'""')}"`).join(",")).join("\n");
  const a = document.createElement("a");
  a.href = "data:text/csv;charset=utf-8," + encodeURIComponent(csv);
  a.download = "inmersion.csv";
  a.click();
});
...
document.getElementById("new-lang-input").addEventListener("keydown", e => {
