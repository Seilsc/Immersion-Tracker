...
function getTotalSeconds() {
  ...
  // Backward compat: count unsaved youtube/show/movie items still in state (legacy data)
  ...
  if (goals.type === "global") {
    ...
    wrap.innerHTML = `
      <div class="goal-bar-top">
        <span class="goal-bar-label"><span class="ic"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg></span> Objetivo diario (global)</span>
    ...
    wrap.innerHTML = `
      <div class="goal-bar-top">
        <span class="goal-bar-label"><span class="ic"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg></span> Objetivo diario · ${currentLang}</span>
        <span class="goal-bar-numbers"><span class="done">${doneStr}</span> / ${goalStr}</span>
      </div>
      <div class="goal-bar-track"><div class="goal-bar-fill${over?' over':''}" style="width:${Math.round(pct*100)}%"></div></div>
      <div class="goal-bar-msg${pct>=1?' complete':''}">${msg}</div>`;
...
function renderMediaTimeControls(idx, type, currentSec, autoSec) {
  const key = `${type}-${idx}`;
  const autoTab = `<button class="time-mode-tab" id="mtab-auto-${key}" onclick="switchMediaTab('${key}','auto')">Automático</button>`;
  return `
...
function renderYoutube() {
  ...
  filtered.forEach((video) => {
    ...
    const actOptions = MEDIA_ACTIVITIES.map(a =>
      `<option value="${a.value}"${video.activity === a.value ? " selected" : ""}>${a.label}${a.manualTime ? " ⏱" : ""}</option>`
    ).join("");
    const actDef = MEDIA_ACTIVITIES.find(a => a.value === video.activity);
    ...
    entry.innerHTML = `
      <div class="show-card-head">
        <div>
          ...
          <div class="show-meta">${video.lang || ""} · duración: ${formatHMS(video.seconds)}</div>
        </div>
      ...
      <div class="show-controls">
        ...
        <select class="show-activity-select" id="yt-act-${realIdx}">${actOptions}</select>
      </div>
      ${isManual ? `
      <div class="show-controls" style="margin-top:0.5rem;">
        <span style="font-family:var(--mono);font-size:11px;color:var(--ink-soft);"><span class="ic"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></span> Actividad interactiva — registra tu tiempo real:</span>
      </div>
      <div class="yt-time-controls" id="yt-time-ctrl-${realIdx}" style="margin-top:0.5rem;">
        ${renderMediaTimeControls(realIdx, 'yt', video.manualSeconds || 0, video.seconds)}
      </div>
      ` : `
      <div class="show-controls" style="margin-top:0.5rem;">
      ...
      `}
      <div style="margin-top:0.75rem;padding-top:0.6rem;border-top:1px solid var(--line);display:flex;align-items:center;gap:0.6rem;flex-wrap:wrap;">
        <button class="accent" style="font-size:13px;padding:0.45rem 1rem;" onclick="saveYtSession(${realIdx})">
          <span class="ic"><svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></span>
          Guardar sesión
        </button>
        <span class="status-msg" id="yt-save-status-${realIdx}" style="margin:0;"></span>
      </div>`;
...
window.saveYtSession = function(i) {
  ...
  state.sessions.push({
    id: Date.now(),
    activityId: "youtube-" + (video.activity || "freeflow-listening"),
    activityName: video.activity || "Freeflow Listening",
    cat: "YouTube",
    lang: video.lang || currentLang,
    note: video.title,
...
async function getTmdbSeasonEpisodes(tvId, season = 1) {
  const res = await tmdbFetch(`/tv/${tvId}/season/${season}`);
  const data = await res.json();
  if (!res.ok) throw new Error(data?.status_message || `HTTP ${res.status}`);
  return (data.episodes || []).filter(e => e.runtime > 0).map(e => e.runtime);
}
...
// Mismas opciones "automáticas" que YouTube/Películas (MEDIA_ACTIVITIES)
  ...
  try {
    ...
    if (runtimes.length === 0) {
      for (let s = 1; s <= (details.number_of_seasons || 1); s++) {
        try {
          runtimes = await getTmdbSeasonEpisodes(result.id, s);
...
function renderShows() {
  ...
  filtered.forEach((show) => {
    ...
    const savedSec = show.manualSeconds || 0;
    ...
    card.innerHTML = `
      ...
      <div class="show-controls" style="margin-top:0.5rem;">
        ...
        <div class="num-stepper">
          <input type="number" id="ep-${i}" min="0" value="${show.episodesWatched || 0}" />
          <div class="num-stepper-btns">
            <button type="button" class="num-stepper-btn plus" data-target="ep-${i}">▴</button>
            <button type="button" class="num-stepper-btn minus" data-target="ep-${i}">▾</button>
          </div>
        </div>
        <span class="show-total" id="show-ep-total-${i}">${formatHMS(epSec)}</span>
      </div>
      ${isManualAct ? `
      <div style="margin-top:0.6rem;">
        <span style="font-family:var(--mono);font-size:11px;color:var(--ink-soft);"><span class="ic"><svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></span> Actividad interactiva — registra tu tiempo real:</span>
        <div style="margin-top:0.4rem;">
          ${renderMediaTimeControls(i, 'show', savedSec, epSec)}
        </div>
      </div>` : ``}
      <div style="margin-top:0.75rem;padding-top:0.6rem;border-top:1px solid var(--line);display:flex;align-items:center;gap:0.6rem;flex-wrap:wrap;">
        <button class="accent" style="font-size:13px;padding:0.45rem 1rem;" onclick="saveShowSession(${i})">
          <span class="ic"><svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></span>
          Guardar sesión
        </button>
        <span class="status-msg" id="show-save-status-${i}" style="margin:0;"></span>
      </div>`;
    ...
    // Al confirmar el valor (blur o Enter), volvemos a dibujar la tarjeta para
    // que el botón "Automático" refleje la duración con el nº de episodios actualizado.
    epInputEl.addEventListener("change", () => {
      renderShows();
    });
  });
}
...
window.saveShowSession = function(i) {
  ...
  if (isManualAct) {
    ...
    if (eps <= 0) {
      if (statusEl) setStatus(statusEl, "Introduce primero los episodios vistos.", "err");
      return;
    }
  ...
  state.sessions.push({
    id: Date.now(),
    activityId: "show-" + (show.activity || "freeflow-listening"),
    activityName: show.activity || "Freeflow Listening",
    cat: "Series",
    lang: show.lang || currentLang,
    note: show.name + epInfo,
...
function renderMovies() {
  ...
  filtered.forEach(movie => {
    ...
    const isManualAct = actDef.manualTime;
    const savedSec = movie.manualSeconds || 0;
    ...
    entry.innerHTML = `
      <div class="show-card-head">
        <div>
          ...
          <div class="show-meta">${movie.lang || ""} · ${movie.year || ""} · duración TMDB: ${formatHMS(movie.seconds)}</div>
        </div>
      ...
      <div style="margin-top:0.6rem;">
        <span style="font-family:var(--mono);font-size:11px;color:var(--ink-soft);"><span class="ic"><svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></span> Actividad interactiva — registra tu tiempo real:</span>
        <div style="margin-top:0.4rem;">
          ${renderMediaTimeControls(realIdx, 'movie', savedSec, movie.seconds)}
        </div>
      </div>` : ``}
      <div style="margin-top:0.75rem;padding-top:0.6rem;border-top:1px solid var(--line);display:flex;align-items:center;gap:0.6rem;flex-wrap:wrap;">
        <button class="accent" style="font-size:13px;padding:0.45rem 1rem;" onclick="saveMovieSession(${realIdx})">
          <span class="ic"><svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></span>
          Guardar sesión
        </button>
        <span class="status-msg" id="movie-save-status-${realIdx}" style="margin:0;"></span>
      </div>`;
    list.appendChild(entry);
...
window.saveMovieSession = function(i) {
  ...
  state.sessions.push({
    id: Date.now(),
    activityId: "movie-" + (movie.activity || "freeflow-listening"),
    activityName: movie.activity || "Freeflow Listening",
    cat: "Películas",
    lang: movie.lang || currentLang,
    note: movie.title + (movie.year ? ` (${movie.year})` : ""),
...
function renderHistory() {
  ...
  // Build full sessions list — youtube/shows/movies are now saved as sessions when "Guardar sesión" is clicked
  // Legacy items still in state.youtube/shows/movies are shown for backward compat
  let allEntries = [];
  ...
  // Legacy: show unsaved youtube/show/movie entries still in state (for users who had data before this update)
  state.youtube.forEach((v, i) => allEntries.push({
    title: v.activity || "YouTube",
    url: v.url || `https://www.youtube.com/watch?v=${v.id}`,
    sub: v.title + " · ⏳ Sin guardar",
    cat: "YouTube",
    lang: v.lang || "",
  ...
  state.shows.forEach((s, i) => {
    ...
    allEntries.push({
      ...
      sub: `${s.name} · ${s.episodesWatched} ep · ⏳ Sin guardar`,
      cat: "Series",
      lang: s.lang || "",
      seconds: secs,
      ts: s.ts || 0,
  ...
  state.movies.forEach((m, i) => allEntries.push({
    ...
    sub: m.title + (m.year ? ` · ${m.year}` : "") + " · ⏳ Sin guardar",
    cat: "Películas",
    lang: m.lang || "",
...
function deleteHistoryEntry(type, idx, btn) {
  // Primera confirmación: cambiar el botón a estado de confirmación
  if (!btn.dataset.confirming) {
    btn.dataset.confirming = "1";
    btn.textContent = "¿Eliminar?";
    btn.classList.add("confirming");
    // Si no se confirma en 3s, volver al estado original
    setTimeout(() => {
      if (btn.dataset.confirming) {
...
async function clearSyncHandle() {
  try { const db = await new Promise(res => { const r = indexedDB.open("SyncHandles", 1); r.onupgradeneeded = e => e.target.result.createObjectStore("handles"); r.onsuccess = () => res(r.result); r.onerror = () => res(null); }); if (!db) return; db.transaction("handles", "readwrite").objectStore("handles").delete("dirHandle"); } catch (_) {}
}
